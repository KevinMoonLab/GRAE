...
class ParametricUMAP(UMAP):
    ...
    def _fit_embed_data(self, X, n_epochs, init, random_state):
        ...
        callback1 = tf.keras.callbacks.EarlyStopping(monitor='val_reconstruction_loss', 
                                                    patience=self.patience)
        
        
        
        callback2 = tf.keras.callbacks.ModelCheckpoint(os.path.join(self.write_path,'checkpoint.h5'), 
                                                       monitor='val_reconstruction_loss')
        
        callback3 = TimeHistory()
...
# X_recon = decoder(encoder(data_train)).numpy()

# fig = plt.figure(figsize=(10,10))
# ax = plt.axes(projection='3d')
# ax.scatter(X_recon[:,0], X_recon[:,1], X_recon[:,2], c = data_t.targets, cmap = "tab10",
#             s = 10, alpha = 0.3)
